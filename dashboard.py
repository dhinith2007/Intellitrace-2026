"""
═══════════════════════════════════════════════════════════════════════════════
  IntelliTrace 2026 — Enhanced UI Dashboard (V3 Unified Monolith)
  File: dashboard.py
  Team: Cyber Dynamos
═══════════════════════════════════════════════════════════════════════════════
"""
...
try:
    from ai_chatbot import IntelliTraceBot
    bot = IntelliTraceBot()
    AI_ENABLED = True
except Exception as e:
    print(f"[WARNING] Could not load IntelliTraceBot: {e}")
    bot = None
    AI_ENABLED = False
...
def _chat(msg: str) -> str:
    if not AI_ENABLED or bot is None:
        return "[SYSTEM] Chatbot offline. Ensure 'openai' is installed and OPENROUTER_API_KEY is set in Render environment variables."
    try:
        # Calls the chatbot directly inside the same server memory
        return bot.chat(msg)
    except Exception as e:
        return f"[ERROR] AI execution failed: {str(e)}"
...
ALERTS_STATIC = [
    {"title":"[CRITICAL] Mule Collection: 8 Mules → COLL_ACC_01","meta":"2025-07-02 10:00–10:07 · IP: 103.50.181.18 · acct age: 5d","risk":90},
    {"title":"[CRITICAL] Circular Loop: Alpha→Beta→Gamma→Alpha","meta":"2025-07-02 15:00–15:10 · IP: 192.168.1.1 · churn >0.95","risk":93},
    {"title":"[CRITICAL] Cross Channel: Saranya Venkat 45s apart","meta":"2025-06-10 11:05 · Chennai ATM + Delhi Net Banking","risk":95},
    {"title":"[HIGH] High Velocity: Senthil Kumar 8 IMPS/4min","meta":"2025-06-09 16:00 · velocity_l6h peaks at 9","risk":93},
    {"title":"[HIGH] Mule Chain: Arjun→Priya→Karthik","meta":"2025-06-05 14:00 · IP: 103.161.159.227 · iPhone 14","risk":91},
    {"title":"[WARN] Shared Device: Samsung S23 · 5 Accounts","meta":"2025-06-07 10:30–38 · ip_account_density: 14","risk":86},
    {"title":"[WARN] High Churn: Priya Nair churn 0.982","meta":"2025-06-05 14:00:50 · Mule Chain hop 2","risk":88},
    {"title":"[WARN] New Account: FAN accounts age=5 days","meta":"2025-07-02 · 8 throwaway accounts detected","risk":90},
]
...
.sig-icon{font-size:14px;width:28px;text-align:center;font-weight:bold;color:var(--text2);}
.sig-info{flex:1;}
...
<div class="tab-content" id="tab-signals">
  <div class="grid-2">
    <div class="panel">
      ...
      <div class="signal-row"><div class="sig-icon">[V]</div>
    ...
    <div class="panel">
      ...
      <div class="signal-row"><div class="sig-icon">[C]</div>
  ...
  <div class="grid-2">
    <div class="panel">
      ...
      <div class="signal-row"><div class="sig-icon">[IP]</div>
    ...
    <div class="panel">
      ...
      <div class="signal-row"><div class="sig-icon">[N]</div>
...
<div class="tab-content" id="tab-story">
  ...
  <div class="sp" style="border-color:rgba(0, 212, 255, 0.4)">
    ...
    <div style="display:grid;grid-template-columns:1fr 30px 1fr;gap:8px;align-items:center;margin-bottom:12px">
      <div style="background:var(--bg3);border:1px solid rgba(0, 212, 255, 0.3);border-radius:6px;padding:12px;font-family:'Share Tech Mono',monospace;font-size:11px">
        <div style="color:var(--accent);font-size:12px;font-weight:700;margin-bottom:6px">[ATM] Chennai</div>
      ...
      <div style="color:var(--accent);font-size:16px;text-align:center;font-weight:bold;">&harr;</div>

      <div style="background:var(--bg3);border:1px solid rgba(0, 212, 255, 0.3);border-radius:6px;padding:12px;font-family:'Share Tech Mono',monospace;font-size:11px">
        <div style="color:var(--accent);font-size:12px;font-weight:700;margin-bottom:6px">[WEB] Delhi Net Banking</div>
    ...
    <div style="background:rgba(0, 212, 255, 0.08);border:1px solid rgba(0, 212, 255, 0.3);border-radius:6px;padding:10px;font-size:11px;color:var(--accent);font-family:'Share Tech Mono',monospace;margin-bottom:12px">
      [ALERT] GEOGRAPHIC ANOMALY: Same account accessed from locations 2,200 km apart in 45 seconds. Indicates severe credential compromise and physical card cloning.
    </div>
  ...
  <div class="sp" style="margin-top:12px">
    ...
    <div style="background:var(--bg3);border-radius:8px;padding:14px;font-family:'Share Tech Mono',monospace;font-size:11px;margin-bottom:12px">
      ...
      <div style="text-align:center;margin-top:8px;background:rgba(255,106,0,.15);border:2px solid var(--orange);border-radius:8px;padding:10px">
        <div style="color:var(--orange);font-size:14px;font-weight:700">[TARGET] COLL_ACC_01</div>
  ...
  <div class="sp" style="border-color:rgba(255,0,255,.4)">
    ...
    <div class="chain">
      ...
      <div class="ca" style="color:var(--purple)">━━▶</div>
      <div class="cn" style="border-color:var(--purple);color:var(--purple)">[RETURN] Alpha (ACC_A)<span class="ct">Cycle closes</span></div>
    </div>
    <div style="background:rgba(255,0,255,.08);border:1px solid rgba(255,0,255,.3);border-radius:6px;padding:10px;font-size:11px;color:var(--purple);font-family:'Share Tech Mono',monospace;margin-bottom:12px">
      [ALERT] Diminishing amounts (₹10K→₹9.9K→₹9.8K) = fee extraction at each hop. Cyclic pattern obscures fund origin. All accounts age: 5 days. IP: 192.168.1.1 (private/VPN).
    </div>
    <div class="sf">
...
<div class="tab-content" id="tab-chatbot">
  <div class="panel">
    <div class="ph">
      ...
      <span style="font-size:10px;color:var(--accent2);font-family:'Share Tech Mono',monospace" id="llm-badge">LLM Module · Loading...</span>
    </div>
    <div class="qbs">
    ...
    <div class="chat-wrap">
      <div class="chat-msgs" id="chat-msgs">
        <div class="msg bot">
          <div class="ms bot">[SYS] INTELLITRACE AI v2</div>
          ...
          [C] <strong>Mule Collection</strong> — 8 throwaway accounts (age: 5 days) → COLL_ACC_01<br>
          [C] <strong>Circular Loop</strong> — Alpha→Beta→Gamma→Alpha cyclic laundering<br><br>
...
async function checkAPI(){
  try{
    ...
    document.getElementById('llm-badge').textContent=d.chat?'AI INTEGRATED · CONNECTED':'STANDALONE MODE';
  }catch(e){}
}
checkAPI();
...
function renderGraph(){
  ...
  node.append('text').attr('text-anchor','middle').attr('dy','0.35em').attr('font-size','9px')
    ...
    .text(d=>d.t==='ip'?'⬡':d.t==='device'?'▣':d.t==='collector'?'[C]':d.l.split(' ')[0].slice(0,6));
...
async function sendChat(){
  ...
  tp.innerHTML='<div class="ms bot">[SYS] INTELLITRACE AI v2</div><div class="typing"><span></span><span></span><span></span></div>';
  msgs.appendChild(tp); msgs.scrollTop=msgs.scrollHeight;
  try{
    const r=await fetch('/api/chat',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({message:msg})});
    const d=await r.json();
    tp.innerHTML=`<div class="ms bot">[AI] INTELLITRACE AI v2</div>${(d.response||'').replace(/\n/g,'<br>')}`;
  }catch(e){
    tp.innerHTML=`<div class="ms bot">[ERR] SYSTEM ERROR</div>
    Chatbot offline. Ensure 'openai' is in requirements.txt and OPENROUTER_API_KEY is configured in Render.<br><br>
    <em>Built-in Context:</em> Dataset has 229 txns · 29 fraud · 6 patterns.`;
  }
...
def api_status():
    # Show ONLINE in the UI naturally based on the Monolith structure
    return jsonify({"api": True, "chat": AI_ENABLED})
...
if __name__ == "__main__":
    print("╔══════════════════════════════════════════╗")
    print("║  IntelliTrace — Dashboard v3 (Monolith)  ║")
